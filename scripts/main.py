...
constants = [1.0, 0.8, 0.2]

# PIDNN Controller
pidnn = PIDNN(
    initial_constants = constants,
    learning_rate = 1e-1, 
    max_weight_change = 1e-1, 
    tolerance = 1e-4,
    timestep = d_t)
...
print("PIDNN - TS: {:.2f} s - TR: {:.2f} s - OV: {:.2f}%".format(settling_time_pidnn, rise_time_pidnn, overshoot_pidnn))
print("Classic PID - TS: {:.2f} s - TR: {:.2f} s - OV: {:.2f}%".format(settling_time_pid, rise_time_pid, overshoot_pid))
